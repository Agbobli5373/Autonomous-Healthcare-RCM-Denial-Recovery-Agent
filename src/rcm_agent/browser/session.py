...
async def cloud_browser(
    ...
    *,
    ...
    stream.emit(
        ...
        kind="tool_call",
        tool="open_browser",
        detail={"key": fingerprint(api_key), "profile": profile_id or "none"},
    )
    ...
    try:
        context = await session.raw.new_context(accept_downloads=DOWNLOADS_ARE_ACCEPTED)
        ...
        stream.emit(
            ...
            tool="open_browser",
            outcome="ok",
            detail={"session": session.id, "expires_at": session.expires_at},
        )
        yield page
    finally:
